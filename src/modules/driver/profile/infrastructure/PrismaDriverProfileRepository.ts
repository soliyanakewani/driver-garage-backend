...
import { DriverProfileDTO } from '../application/dto/profile.dto';
...
export class PrismaDriverProfileRepository implements DriverProfileRepository {
    ...
    async getProfile(driverId: string) {
      ...
      async createProfile(driverId: string, data: DriverProfileDTO) {
      ...
      async updateProfile(driverId: string, data: DriverProfileDTO) {
        const existing = await prisma.driver.findUnique({ where: { id: driverId } });
        if (!existing) throw new Error('Driver not found');
